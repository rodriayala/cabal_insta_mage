<?php
class EM_Em0139settings_Model_Observer
{
    public function beforeGenerateBlocks(Varien_Event_Observer $observer){
        if ((Mage::getSingleton('core/design_package')->getPackageName() == 'em0139') &&
            ...
            if (Mage::helper('themeframework/settings')->getGeneral_DisableDefaultNav()!=1 && (Mage::
                ...
                $blocks = $observer->getLayout()->getXpath('//block[@name="em0139.catalog.topnav"]');
                if (!empty($blocks))
    ...
    public function addItemEvent($observer){
        $observer->getHead()->addCSS('em0139/em0139.css');
        $observer->getHead()->addItem('skin_js', 'em0139/em0139.js');
        return $this;
    }
}
