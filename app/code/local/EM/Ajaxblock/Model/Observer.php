...
class EM_Ajaxblock_Model_Observer
{
	public function htmlBefore(Varien_Event_Observer $observer)
	{
	    ...
	    else
	    {
			...
			if(get_class($block) == 'EM_Minifilterproducts_Block_Listcustom')
            {
                $data = base64_encode(json_encode($block->getData()));
                $block->setData('data',$data);
                $block->setData('custom_theme','ajaxblock/ajaxtab.phtml');
            }
			if(get_class($block) == 'EM_Multidealpro_Block_Widgets')
            {
                $data = base64_encode(json_encode($block->getData()));
                $block->setData('data',$data);
                $block->setData('custom_theme','ajaxblock/index_deals.phtml');
            }
        }
	}
}
